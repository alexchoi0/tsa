...
use super::super::entity::password_history::{ActiveModel, Column, Entity};
...
impl From<super::super::entity::password_history::Model> for PasswordHistory {
    fn from(model: super::super::entity::password_history::Model) -> Self {
        PasswordHistory {
            id: model.id,
...
impl PasswordHistoryRepository for SeaOrmPasswordHistoryRepository {
    ...
    async fn delete_old_entries(&self, user_id: Uuid, keep_count: u32) -> Result<u64> {
        let all_entries: Vec<super::super::entity::password_history::Model> = Entity::find()
            .filter(Column::UserId.eq(user_id))
            .order_by_desc(Column::CreatedAt)
            .all(self.db.as_ref())
            .await
